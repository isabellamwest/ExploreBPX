...
- **Provenance & confidence per value** `[deferred]` — sidecar recording where
  ...
  the command's metadata honest so provenance can attach later. *(2026-08-03:
  the reference grid / overlay work is now a second natural producer; candidate
  for promotion once the multi-file track ships.)*
