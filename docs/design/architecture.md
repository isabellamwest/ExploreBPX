...
> **Design record.** This document was deleted when its essentials were folded
> into the README (commit `da340a8`) and has been restored from git history as
> a record of the design; it is not maintained. Where it and the running app
> disagree, the app is correct. The README carries the current essentials.
