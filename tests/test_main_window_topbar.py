...
def test_identity_shows_title_model_and_version(app_driver, valid_spm_path):
    app_driver.open(valid_spm_path)

    assert app_driver.identity_text() == "Minimal valid SPM example · SPM · BPX v1.0.0"
...
def test_identity_falls_back_to_filename_when_title_empty(
    ...
    assert app_driver.identity_text() == "no_title_example.json · SPM · BPX v1.0.0"


def test_identity_omits_missing_model_without_stray_separator(
    app_driver, valid_spm_dict, tmp_path
):
