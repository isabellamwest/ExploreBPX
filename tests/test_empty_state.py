...
Covers three surfaces:
  - the Validation activity-bar badge (plain "Validation", never "(0)")
...
def test_validation_badge_shows_no_count_with_no_document(app_driver):
    assert app_driver.validation_badge_text() == "Validation"
...
def test_validation_badge_shows_count_once_an_issue_exists(app_driver, spm_workfile):
    ...
    assert d.validation_badge_text() != "Validation"
    assert d.validation_badge_text().startswith("Validation (")
